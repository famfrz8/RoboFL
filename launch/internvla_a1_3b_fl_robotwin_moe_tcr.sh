...
###############################################################################
########### RoboTwin Federated Learning + LoRA-MoE + TCR Launch Script #########
#                                                                             #
# Default: LoRA-MoE FL with prototype-guided TCR regularization               #
# GPU, client, round and loss weights can be overridden via env vars                         #
###############################################################################
...
JOB_NAME="${JOB_NAME:-$(date +'%Y_%m_%d_%H_%M_%S')-${POLICY}-fl-moe-tcr-${FL_NUM_CLIENTS}clients-r${LORA_RANK}}"
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_OUTPUT_DIR}/${JOB_NAME}}"
export FL_OUTPUT_DIR="${OUTPUT_DIR}/global_model"

echo "=============================================="
echo "ROBOTWIN FL + LoRA-MoE + TCR"
echo "=============================================="
...
echo "  TCR: enable=${TCR_ENABLE}, lambda_proto=${TCR_LAMBDA_PROTO}, lambda_contrast=${TCR_LAMBDA_CONTRAST}, loss_weight=${TCR_LOSS_WEIGHT}"
...
ARGS=(
    ...
    --wandb.project="${WANDB_PROJECT:-lerobot_fl_robotwin_moe_tcr}"
...
#
# Quick smoke test:
#   FL_NUM_ROUNDS=1 FL_LOCAL_STEPS=1 MOE_STEPS=1 NUM_GPUS=1 bash launch/internvla_a1_3b_fl_robotwin_moe_tcr.sh
#
# Tune TCR weights:
#   TCR_LAMBDA_PROTO=0.2 TCR_LAMBDA_CONTRAST=1.0 bash launch/internvla_a1_3b_fl_robotwin_moe_tcr.sh
#
# Specify dataset:
#   DATASET_REPO_ID=robotwin/aloha-example bash launch/internvla_a1_3b_fl_robotwin_moe_tcr.sh
#
###############################################################################
