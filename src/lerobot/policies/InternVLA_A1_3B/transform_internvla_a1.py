...
class Qwen3_VLProcessorTransformFn(DataTransformFn):
    ...
    def __call__(self, data: DataDict) -> DataDict: 
        ...
        pixel_values = []
        image_grid_thw = []
        valid_image_inputs = None
        valid_num_img_token = None
        for i in range(3):
            ...
            if data[f"{k}_mask"]:
                ...
                valid_image_inputs = img_inputs
                valid_num_img_token = num_img_token
                # attention_mask += [0] + [1] * num_img_token + [0]
            else:
                if valid_image_inputs is None:
                    raise ValueError("At least one image view must be enabled")
                pixel_values.append(valid_image_inputs.pixel_values)
                image_grid_thw.append(valid_image_inputs.image_grid_thw)
                input_ids += [self.vision_start_token_id] + [self.image_token_id] * valid_num_img_token + [self.vision_end_token_id]
                attention_mask += [0] * (valid_num_img_token + 2)
...
class UnifyQwenA1InputsTransformFn(DataTransformFn):
    def __call__(self, data: DataDict) -> DataDict: 
        action_pad_keys = (
            "actions.effector.position_is_pad",
            "actions.gripper.position_is_pad",
            "actions.joint.position_is_pad",
        )
        action_is_pad = None
        for key in action_pad_keys:
            if key in data:
                mask = data[key].bool()
                action_is_pad = mask if action_is_pad is None else action_is_pad | mask
...
if __name__ == "__main__":
    sample = {
        ...
        f"{OBS_IMAGES}.image0_mask": True,    # Add mask
        f"{OBS_IMAGES}.image1_mask": True,    # Add mask
        f"{OBS_IMAGES}.image2_mask": True,    # Add mask
