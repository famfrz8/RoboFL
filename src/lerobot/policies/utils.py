...
def log_model_loading_keys(missing_keys, unexpected_keys) -> None:
    ...
    """
    if missing_keys:
        preview = sorted(missing_keys)[:20]
        suffix = " ..." if len(missing_keys) > 20 else ""
        logging.warning(f"Missing key(s) when loading model ({len(missing_keys)}): {preview}{suffix}")
    if unexpected_keys:
        preview = sorted(unexpected_keys)[:20]
        suffix = " ..." if len(unexpected_keys) > 20 else ""
        logging.warning(f"Unexpected key(s) when loading model ({len(unexpected_keys)}): {preview}{suffix}")
