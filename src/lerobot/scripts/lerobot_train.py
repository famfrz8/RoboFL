...
def update_policy(
    ...
    if "loss_aux" in output_dict:
        train_metrics.loss_aux = output_dict["loss_aux"]
...
def train(cfg: TrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    # Bind each DDP rank before constructing the policy. Without this, the
    # policy config may resolve to cuda:0 for every rank during checkpoint load.
    if torch.cuda.is_available():
        ...
        if local_rank >= device_count:
            raise RuntimeError(
                ...
                "Check CUDA_VISIBLE_DEVICES and --num_processes."
            )
        torch.cuda.set_device(local_rank)
        cfg.policy.device = f"cuda:{local_rank}"
        print(
            f"[DDP device] rank={os.environ.get('RANK', '0')} "
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step:
            ...
            if wandb_logger:
                # Use to_dict() to get the averaged log values over the last _freq steps
                wandb_log_dict = train_tracker.to_dict()
                # DEBUG: print the average for verification
                logging.debug(f"[WandB Debug] train_tracker.to_dict() averages: loss={wandb_log_dict.get('loss', 'N/A')}, loss_action={wandb_log_dict.get('loss_action', 'N/A')}, loss_gen={wandb_log_dict.get('loss_gen', 'N/A')}, loss_aux={wandb_log_dict.get('loss_aux', 'N/A')}")
                if output_dict:
                    # Only add keys missing from output_dict to avoid overwriting averages
                    # Raw values are stored under keys with a _step suffix for comparison
                    for k, v in output_dict.items():
                        if k not in wandb_log_dict:
                            wandb_log_dict[k] = v
                        else:
                            wandb_log_dict[f"{k}_step"] = v  # Raw value of the current batch
                    # DEBUG: print the raw value of the current batch
                    logging.debug(f"[WandB Debug] output_dict raw values: loss={output_dict.get('loss', 'N/A')}, loss_action={output_dict.get('loss_action', 'N/A')}, loss_gen={output_dict.get('loss_gen', 'N/A')}, loss_aux={output_dict.get('loss_aux', 'N/A')}")
