...
#   FL_NUM_CLIENTS=4
#   FL_CLIENT_GPUS=1 (2 requires eight ranks, four clients/experts; batch_size is per client)
#   FL_LOCAL_STEPS=100
#   FL_NUM_ROUNDS=50
...
from lerobot.utils.grouped_client import (
    average_client_metrics,
    client_sampler,
    create_client_group,
    cycle_client_batches,
    global_client_sampler,
    unwrap_client_policy,
    validate_client_gpus,
    validate_sequential_clients,
    wrap_client_policy,
)
...
def get_fl_config():
    ...
    return {
        ...
        "client_gpus": int(os.environ.get("FL_CLIENT_GPUS", "1")),
        "sequential_clients": os.environ.get("FL_SEQUENTIAL_CLIENTS", "false").lower() == "true",
...
def get_trainable_weights(policy):
    ...
    policy = unwrap_client_policy(policy)
    import logging
    logger = logging.getLogger()
    trainable_state = {}
    for name, param in policy.named_parameters():
...
def fl_train(cfg: TrainPipelineConfig):
    ...
    if fl_cfg["partition_strategy"] == "intern_real_by_task":
        ...
        sequential = bool(fl_cfg.get("sequential_clients", False))
        if sequential:
            valid_real_layout = (
                fl_cfg["num_clients"] == 2
                and int(cfg.policy.loramoe_num_experts) == 2
                and fl_cfg["num_gpus"] == world_size == 8
            )
            expert_error = (
                "Sequential real-robot FL requires two clients and two complete A/B experts "
                "on eight ranks"
            )
        else:
            valid_real_layout = (
                fl_cfg["num_clients"] == 4
                and fl_cfg["num_gpus"] == world_size
                and ((1 <= world_size <= 4 and fl_cfg["client_gpus"] == 1)
                     or (world_size == 8 and fl_cfg["client_gpus"] == 2))
            )
            expert_error = "Real-robot FL requires four complete A/B experts, top-k 2, no shared A/AB router"
        if not valid_real_layout:
            raise ValueError(
                "Real-robot FL requires four clients with one GPU per client (1..4 ranks), "
                "FL_CLIENT_GPUS=2 on eight ranks, or FL_SEQUENTIAL_CLIENTS=true with two "
                "clients/two experts on eight ranks"
            )
        if (cfg.policy.loramoe_num_experts != (2 if sequential else 4)
                or not 1 <= int(cfg.policy.loramoe_router_top_k) <= int(cfg.policy.loramoe_num_experts)
    ...
    client_gpus = fl_cfg["client_gpus"]
    validate_client_gpus(
        client_gpus, world_size, num_clients, int(cfg.policy.loramoe_num_experts), cfg.batch_size
    )
    if fl_cfg.get("sequential_clients", False):
        validate_sequential_clients(
            world_size, num_clients, int(cfg.policy.loramoe_num_experts), cfg.batch_size
        )
    client_group = None
    client_group_rank = 0
    if client_gpus == 2:
        client_group, grouped_client_id, client_group_rank = create_client_group()
    if fl_cfg["partition_strategy"] == "intern_real_by_task":
        from lerobot.datasets.intern_real_dataset import partition_intern_real
    ...
    if fl_cfg["partition_strategy"] == "intern_real_by_task":
        from lerobot.datasets.intern_real_dataset import persist_intern_real_artifacts
    ...
    if client_gpus == 2:
        my_client_ids = [grouped_client_id]
    # Followers retain uploads for Phase 5/reload, but never count as another client.
    contributing_client_ids = my_client_ids if client_group_rank == 0 else []
    if client_gpus == 2 and local_rank == 0:
        logger.info(
            f"[ClientDDP] groups=[[0,4],[1,5],[2,6],[3,7]], leaders=[0,1,2,3], "
            f"batch_per_client={cfg.batch_size}, batch_per_rank={cfg.batch_size // 2}; "
            "server uses all eight ranks"
        )
    if fl_cfg.get("sequential_clients", False):
        # Every rank trains every client in turn; no client owns a GPU subset.
        my_client_ids = list(range(num_clients))
        contributing_client_ids = my_client_ids if local_rank == 0 else []
        if local_rank == 0:
            logger.info(
                f"[SequentialClients] clients trained one at a time on all {world_size} ranks; "
                f"per-rank batch={cfg.batch_size}"
            )
    ...
    if fl_cfg["partition_strategy"] == "intern_real_by_task":
        # Keep the partition_intern_real selected during preflight.
        pass
    elif fl_cfg["partition_strategy"] == "mole_by_task":
    ...
    if fl_cfg.get("sequential_clients", False):
        # Snapshot the shared round-start weights so a client without a carried
        # state does not inherit the previously trained client's update when all
        # clients are trained in sequence on the same ranks.
        round_initial_client_state = get_trainable_weights(client_policy)
    ...
    for client_id in my_client_ids:
        ...
        if fl_cfg.get("sequential_clients", False):
            # All ranks shard one client partition; every rank sees distinct data.
            # cfg.batch_size is the PER-RANK batch here (each client is trained by
            # all world_size ranks), so the per-client total is cfg.batch_size *
            # world_size. This differs from the client_gpus>1 modes where
            # cfg.batch_size is the per-client total split across client_gpus.
            sampler = global_client_sampler(ds, local_rank, world_size, fl_cfg["seed"])
            ...
            client_dataloaders[client_id] = iter(cycle_client_batches(dl))
            logger.info(f"[Rank {local_rank}] Client {client_id}: {len(ds)} samples initialized")
            continue
        sampler = client_sampler(ds, client_group_rank, fl_cfg["seed"]) if client_gpus == 2 else None
        dl = torch.utils.data.DataLoader(
            ds,
            batch_size=cfg.batch_size // client_gpus,
        ...
        client_dataloaders[client_id] = iter(cycle_client_batches(dl) if client_gpus == 2 else cycle(dl))
        logger.info(f"[Rank {local_rank}] Client {client_id}: {len(ds)} samples initialized")
    ...
    if loramoe_enabled:
        if fl_cfg["partition_strategy"] == "intern_real_by_task":
            moe_dataset = partitioner(dataset, num_clients=num_clients)
        elif fl_cfg["partition_strategy"] == "mole_by_task":
    ...
    for fl_round in range(fl_cfg["num_rounds"]):
        ...
        for client_id in my_client_ids:
            ...
            if fl_cfg.get("sequential_clients", False) and client_id not in client_states:
                # No carried state yet: reset to the shared round-start weights so
                # this client does not start from the previously trained client.
                set_trainable_weights(client_policy, round_initial_client_state)
            ...
            if client_gpus == 2 and lr_scheduler is not None:
                lr_scheduler.load_state_dict(deepcopy(
                    client_scheduler_states.get(client_id, initial_lr_scheduler_state)
                ))

            # Train client
            local_training_policy = (
                wrap_client_policy(client_policy, client_group) if client_gpus == 2 else client_policy
            )
            if fl_cfg.get("sequential_clients", False):
                # All ranks cooperate on this single client via the default process group.
                local_training_policy = wrap_client_policy(client_policy, None)
            train_result = run_local_training(
                policy=local_training_policy,
            ...
            # Release reducer/buckets before CPU offload or the global server phase.
            del local_training_policy
            if client_gpus == 2:
                train_result["metrics"] = average_client_metrics(train_result["metrics"], client_group, device)
            if fl_cfg.get("sequential_clients", False):
                # Metrics are sharded across ranks; average over the whole group for logging.
                train_result["metrics"] = average_client_metrics(train_result["metrics"], None, device)
            if client_id in contributing_client_ids:
                all_client_metrics.append(train_result["metrics"])

            # Save to client_states
            client_states[client_id] = train_result["model_state"]
            # =====================================================================
            ...
            if client_gpus == 2 and lr_scheduler is not None:
                client_scheduler_states[client_id] = deepcopy(lr_scheduler.state_dict())
        ...
        if loramoe_enabled:
            ...
            for client_id in contributing_client_ids:
                if client_id in client_states:
                    client_state = client_states[client_id]
                    action_head_state = {}
                    for name, param in client_state.items():
        ...
        if loramoe_enabled:
            ...
            for client_id in my_client_ids:
                ...
                # In grouped mode both members start from the globally trained server
                # and mix the same expert, retaining identical next-round local states.
