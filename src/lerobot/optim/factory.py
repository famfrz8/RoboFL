...
def make_optimizer_and_scheduler(
    ...
    # Check if using policy training preset
    logger.info(f"[make_optimizer_and_scheduler] use_policy_training_preset = {cfg.use_policy_training_preset}")

    if cfg.use_policy_training_preset:
        params = policy.get_optim_params()
        logger.info(f"[make_optimizer_and_scheduler] Using policy.get_optim_params() - got {len(params)} param groups")
    else:
        params = policy.parameters()
        logger.info(f"[make_optimizer_and_scheduler] Using policy.parameters()")
